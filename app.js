...
program
    ...
    .option(
        ...
        "wss://ropsten-rpc.linkpool.io/ws"
    )
    .option(
        "-t, --tip <percent>",
        "The percentage of mined coins to tip the developers",
        "5"
    )
    .option(
        "-p, --proof-period <seconds>",
        "How often you want to submit a proof on average",
        "86400"
    )
    .option(
        "-k, --key-file <file>",
        "AES encrypted file containing private key"
    )
    .option(
        "-m, --gas-multiplier <multiplier>",
        "The multiplier to apply to the recommended gas price",
        "1"
    )
    .option(
        "-l, --gas-price-limit <limit>",
...
let hashrateCallback = function (hashrate) {
    console.log(
        `[JS](app.js) Hashrate: ` + KoinosMiner.formatHashrate(hashrate)
    );
};
...
let signCallback = async function (web3, txData) {
    return (await web3.eth.accounts.signTransaction(txData, account.privateKey))
        .rawTransaction;
};

function enterPassword() {
    return readlineSync.questionNewPassword("Enter password for encryption: ", {
        mask: "",
    });
}
...
function encrypt(data, password) {
    ...
    const iv = Buffer.from(
        crypto.createHmac("sha256", passwordHash).digest("hex").slice(32),
        "hex"
    );
...
if (program.import) {
    ...
    if (
        readlineSync.keyInYNStrict(
            "Do you want to store your private key encrypted on disk?"
        )
    ) {
        var cipherText = encrypt(account.privateKey, enterPassword());

        var filename = readlineSync.question(
            "Where do you want to save the encrypted private key? "
        );
    ...
    if (
        program.export &&
        !readlineSync.keyInYNStrict(
            "Outputting your private key unencrypted can be dangerous. Are you sure you want to continue?"
        )
    ) {
        process.exit(0);
    }
    ...
    if (
        !readlineSync.keyInYNStrict(
            "No private key file specified. Do you want to create a new key?"
        )
    ) {
        process.exit(0);
    }

    var seed = readlineSync.question("Enter seed for entropy: ", {
        hideEchoBack: true,
        mask: "",
    });
    account = w3.eth.accounts.create(
        crypto.createHmac("sha256", seed).digest("hex")
    );
