...
void bignum_to_string(struct bn* n, char* str, int nbytes, bool leading_zeros)
{
  require(n, "n is null");
  require(str, "str is null");
  require(nbytes > 0, "nbytes must be positive");
...
void bignum_endian_swap(struct bn* n )
{
#ifdef WORD_SIZE
  int i;
  for (i = 0; i < BN_ARRAY_SIZE; ++i)
  {
   #if (WORD_SIZE == 1)
     /* Do nothing, already endianness doesn't matter */
   #elif (WORD_SIZE == 2)
    n->array[i] = ((n->array[i] << 8) & 0xff00 ) | // Move byte 0 to byte 1
                  ((n->array[i] >> 8) & 0x00ff );  // Move byte 1 to byte 0
   #elif (WORD_SIZE == 4)
    n->array[i] = ((n->array[i] << 24) & 0xff000000 ) | // Move byte 0 to byte 3
                  ((n->array[i] << 8)  & 0x00ff0000 ) | // Move byte 1 to byte 2
                  ((n->array[i] >> 8)  & 0x0000ff00 ) | // Move byte 2 to byte 1
                  ((n->array[i] >> 24) & 0x000000ff );  // Move byte 3 to byte 0
   #endif
  }

  for (i = 0; i < BN_ARRAY_SIZE / 2; ++i )
  {
     // Swap n->array[i] and n->array[BN_ARRAY_SIZE - i - 1]
     n->array[i] ^= n->array[BN_ARRAY_SIZE - i - 1];
     n->array[BN_ARRAY_SIZE - i - 1] ^= n->array[i];
     n->array[i] ^= n->array[BN_ARRAY_SIZE - i - 1];
  }
#endif
}
