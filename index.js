...
function difficultyToString(difficulty) {
    ...
    difficultyStr =
        "0x" + "0".repeat(64 - difficultyStr.length) + difficultyStr;
...
class MiningRequestQueue {
    ...
    getHead() {
        if (this.pendingRequests.length === 0) return null;
...
module.exports = class KoinosMiner {
    ...
    constructor(
        ...
        this.updateBlockchainLoop = new Looper(
            function () {
                return self.updateBlockchain();
            },
            60 * 1000,
            function (e) {
                return self.updateBlockchainError(e);
            }
        );
        ...
        this.contractStartTimePromise = this.contract.methods
            .start_time()
            .call()
            .then((startTime) => {
    ...
    sendTransaction(txData) {
        ...
        self.signCallback(self.web3, txData).then((rawTx) => {
            self.web3.eth
                ...
                .then((receipt) => {
                    console.log(
                        "[JS] Transaction hash is",
                        receipt.transactionHash
                    );
                    if (
                        self.proofCallback &&
                        typeof self.proofCallback === "function"
                    ) {
                ...
                .catch(async (e) => {
                    ...
                    if (
                        ...
                        typeof self.warningCallback === "function"
                    ) {
    ...
    getTipAddressesForMiner(minerAddress) {
        ...
        for (let i = 0; i < this.tipAddresses.length; i++) {
            ...
            let sortKey = this.web3.utils.soliditySha3(
                minerAddress,
                tipAddress
            );
    ...
    async retry(maxRetries, msg, fn) {
        ...
        while (tries < maxRetries) {
            try {
                ...
                tries++;
                console.log(
                    "[JS] Attempting to " + msg + ", attempts: " + tries
                );
                await sleep(3000);
                if (tries == maxRetries) {
                    throw e;
    ...
    async onRespNonce(req, nonce) {
        ...
        let gasPrice = Math.round(
            parseInt(await this.web3.eth.getGasPrice()) * this.gasMultiplier
        );
        ...
        if (gasPrice > this.gasPriceLimit) {
            let error = {
                kMessage:
                    "The gas price (" +
                    gasPrice +
                    ") has exceeded the gas price limit (" +
                    this.gasPriceLimit +
                    ").",
            };
            if (
    ...
    async runMiner() {
        ...
        this.child.stdout.on("data", async function (data) {
            if (self.isFinishedWithoutNonce(data)) {
                ...
                await self.onRespHashReport(
                    self.miningQueue.getHead(),
                    newHashes
                );
            } else {
                let error = {
                    kMessage:
                        "Unrecognized response from the C mining application.",
                };
                if (
                    self.errorCallback &&
                    typeof self.errorCallback === "function"
    ...
    async start() {
        ...
        if (now < this.contractStartTime) {
            ...
            console.log(
                "[JS] Mining will begin at " + startDateTime.toLocaleString()
            );
    ...
    updateHashrate(d_hashes, d_time) {
        ...
        if (
            this.hashrateCallback &&
            typeof this.hashrateCallback === "function"
        ) {
    ...
    adjustDifficulty() {
        ...
        this.threadIterations = Math.max(
            this.hashRate / (2 * os.cpus().length),
            1
        ); // Per thread hash rate, sync twice a second
    ...
    static formatHashrate(h) {
        ...
        switch (Math.trunc(Math.log10(h) / 3)) {
            ...
            case 1:
                return (
                    Math.trunc(h / 1000) + "." + Math.trunc(h % 1000) + " KH/s"
                );
            case 2:
                return (
                    Math.trunc(h / 1000000) +
                    "." +
                    Math.trunc((h / 1000) % 1000) +
                    " MH/s"
                );
            default:
                return (
                    Math.trunc(h / 1000000000) +
                    "." +
                    Math.trunc((h / 1000000) % 1000) +
                    " GH/s"
                );
        }
    }
    ...
    getNonceOffset() {
        ...
        maxOffsetStr =
            "0x" + "0".repeat(64 - maxOffsetStr.length) + maxOffsetStr;
    ...
    async updateLatestBlock() {
        try {
            ...
            let confirmedBlock = await this.web3.eth.getBlock(
                this.headBlock.number - 6
            );
            this.recentBlock = confirmedBlock;
        } catch (e) {
            let error = {
                kMessage:
                    "An error occurred while attempting to retrieve the latest block.",
                exception: e,
            };
            throw error;
        }
